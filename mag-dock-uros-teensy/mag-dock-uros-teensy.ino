...
IntervalTimer myErrorTimer;             // Create an interval timer
volatile bool errorTimerRunning = false;         // Flag to indicate if the timer is running
unsigned long errorTimerPeriod_ms = 0; // Debounce delay in milliseconds

void error_loop(const char* function_name){
  Serial.begin(115200);
  startErrorTimer(100);
  while(1){
    delay(100);
    Serial.write("Help!\n");
    Serial.write(function_name);
    Serial.write(" Help!\n");
    if(init_rcl()) {
      endErrorTimer();
      return;
...
void startErrorTimer(const unsigned long newtime) {
    // Start the timer if it's not already running
    if (newtime != errorTimerPeriod_ms) {
      errorTimerPeriod_ms = newtime;
      myErrorTimer.end();
    }
    myErrorTimer.begin(errorTimerCallback, errorTimerPeriod_ms * 1000); // Convert to microseconds
}
